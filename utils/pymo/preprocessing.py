...
class MocapParameterizer(BaseEstimator, TransformerMixin):
    ...
    def _to_quat(self, X):
        ...
        for track in X:
            ...
            rot_cols = [
                c for c in euler_df.columns if ("rotation" in c and "Nub" not in c)
            ]
            ...
            for joint in joints:
                ...
                quat_df["%s_qWrotation" % joint] = pd.Series(
                    data=[e[0] for e in quats], index=quat_df.index
                )
                quat_df["%s_qXrotation" % joint] = pd.Series(
                    data=[e[1] for e in quats], index=quat_df.index
                )
                quat_df["%s_qYrotation" % joint] = pd.Series(
                    data=[e[2] for e in quats], index=quat_df.index
                )
                quat_df["%s_qZrotation" % joint] = pd.Series(
                    data=[e[3] for e in quats], index=quat_df.index
                )
    ...
    def _quat_to_euler(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                r = quat_df[
                    [c for c in quat_params if joint in c]
        ...
        for track in X:
            ...
            for joint in track.traverse():
                    ...
                    euler_values = (
                        ...
                        * np.transpose(
                            np.array(
                                [
                                    track.values[
                                        "%s_%srotation" % (joint, rot_order[0])
                                    ],
                                    track.values[
                                        "%s_%srotation" % (joint, rot_order[1])
                                    ],
                                    track.values[
                                        "%s_%srotation" % (joint, rot_order[2])
                                    ],
                                ]
                            )
                        )
                ...
                if track.root_name == joint:
                    ...
                    tree_data[joint][0] = (
                        tree_data[parent][0] * quats
                    )  # np.matmul(rotmats, tree_data[parent][0])
                    ...
                    )  # q.reshape(k.shape[0],3) + tree_data[parent][1]
    ...
    def _expmap_to_pos(self, X):
        ...
        for track in X:
            ...
            for joint in track.traverse():
                ...
                if "Nub" not in joint:
                    r = exp_df[
                        [c for c in exp_params if joint in c]
                    ]  # Get the columns that belong to this joint
                    expmap = r.values
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                expmap = np.transpose(
                    np.array(
                        [
                            track.values[jt_alpha],
                            track.values[jt_beta],
                            track.values[jt_gamma],
                        ]
                    )
                )
                rot_order = track.skeleton[joint]["order"]
                euler_rots = np.array(
                    R.from_rotvec(expmap).as_euler(rot_order, degrees=True)
                )
                # euler_rots = [expmap2euler(f, rot_order, True) for f in expmap] # Convert the exp maps to eulers
    ...
    def _expmap_to_euler2(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler_df.drop(
                    ["%s_alpha" % joint, "%s_beta" % joint, "%s_gamma" % joint],
                    axis=1,
                    inplace=True,
                )
                expmap = [
                ...
                quats = Quaternions.from_angle_axis(
                    angs, expmap / (np.tile(angs[:, None] + 1e-10, (1, 3)))
                )
                if self.ref_pose is not None:
                    q1_col = "%s_qWrotation" % (joint)
                    q2_col = "%s_qXrotation" % (joint)
    ...
    def _euler_to_vectors(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler = [
                    [f[1][r1_col], f[1][r2_col], f[1][r3_col]] for f in r.iterrows()
                ]
                ...
                vec_df.insert(
                    loc=0,
                    column="%s_xUp" % joint,
                    value=pd.Series(data=[e[0] for e in vectors], index=vec_df.index),
                )
                vec_df.insert(
                    loc=0,
                    column="%s_yUp" % joint,
                    value=pd.Series(data=[e[1] for e in vectors], index=vec_df.index),
                )
                vec_df.insert(
                    loc=0,
                    column="%s_zUp" % joint,
                    value=pd.Series(data=[e[2] for e in vectors], index=vec_df.index),
                )
                vec_df.insert(
                    loc=0,
                    column="%s_xFwd" % joint,
                    value=pd.Series(data=[e[3] for e in vectors], index=vec_df.index),
                )
                vec_df.insert(
                    loc=0,
                    column="%s_yFwd" % joint,
                    value=pd.Series(data=[e[4] for e in vectors], index=vec_df.index),
                )
                vec_df.insert(
                    loc=0,
                    column="%s_zFwd" % joint,
                    value=pd.Series(data=[e[5] for e in vectors], index=vec_df.index),
                )
    ...
    def _vectors_to_euler(self, X):
        ...
        for track in X:
            ...
            vec_params = [
                ...
                if (
                    any(p in c for p in ["xUp", "yUp", "zUp", "xFwd", "yFwd", "zFwd"])
                    and "Nub" not in c
                )
            ]
            ...
            for joint in joints:
                r = vec_df[
                    [c for c in vec_params if joint in c]
                ]  # Get the columns that belong to this joint
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            lft_joints = (
                ...
                if "Left" in joint and "Nub" not in joint
            )
            rgt_joints = (
                joint
                for joint in track.skeleton
                if "Right" in joint and "Nub" not in joint
            )
            ...
            for lft_joint in lft_joints:
                ...
                new_df["%s_Xrotation" % lft_joint] = pd.Series(
                    data=signs[0] * track.values["%s_Xrotation" % rgt_joint],
                    index=new_df.index,
                )
                new_df["%s_Yrotation" % lft_joint] = pd.Series(
                    data=signs[1] * track.values["%s_Yrotation" % rgt_joint],
                    index=new_df.index,
                )
                new_df["%s_Zrotation" % lft_joint] = pd.Series(
                    data=signs[2] * track.values["%s_Zrotation" % rgt_joint],
                    index=new_df.index,
                )

                new_df["%s_Xrotation" % rgt_joint] = pd.Series(
                    data=signs[0] * track.values["%s_Xrotation" % lft_joint],
                    index=new_df.index,
                )
                new_df["%s_Yrotation" % rgt_joint] = pd.Series(
                    data=signs[1] * track.values["%s_Yrotation" % lft_joint],
                    index=new_df.index,
                )
                new_df["%s_Zrotation" % rgt_joint] = pd.Series(
                    data=signs[2] * track.values["%s_Zrotation" % lft_joint],
                    index=new_df.index,
                )
            ...
            joints = (
                joint
                for joint in track.skeleton
                if "Nub" not in joint and "Left" not in joint and "Right" not in joint
            )
            ...
            for joint in joints:
                ...
                new_df["%s_Xrotation" % joint] = pd.Series(
                    data=signs[0] * track.values["%s_Xrotation" % joint],
                    index=new_df.index,
                )
                new_df["%s_Yrotation" % joint] = pd.Series(
                    data=signs[1] * track.values["%s_Yrotation" % joint],
                    index=new_df.index,
                )
                new_df["%s_Zrotation" % joint] = pd.Series(
                    data=signs[2] * track.values["%s_Zrotation" % joint],
                    index=new_df.index,
                )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                new_euler = [
                    euler_reorder(f, rot_order, self.new_order, True) for f in euler
                ]
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for joint_name in selected_joints:
            if joint_name.endswith("_Nub"):
                selected_channels.extend(
                    [o for o in X[0].values.columns if (joint_name + "_") in o]
                )
            else:
                selected_channels.extend(
                    [
                        o
                        for o in X[0].values.columns
                        if (joint_name + "_") in o and "Nub" not in o
                    ]
                )
        ...
        self.not_selected_values = {
            c: X[0].values[c].values[0] for c in self.not_selected
        }
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            for key in t2.skeleton.keys():
                to_remove = list(
                    set(t2.skeleton[key]["children"]) - set(self.selected_joints)
                )
...
class Numpyfier(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            ...
            time_index = (
                pd.to_timedelta([f for f in range(track.shape[0])], unit="s")
                * self.org_mocap_.framerate
            )

            new_df = pd.DataFrame(
                data=track, index=time_index, columns=self.org_mocap_.values.columns
            )
...
class Slicer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            n_sequences = (nframes - overlap_frames) // (
                self.window_size - overlap_frames
            )
...
class RootTransformer(BaseEstimator, TransformerMixin):
    def __init__(
        ...
        for i in range(3):
            ...
            rot_mat[i, ax_i] = 1
        self.root_rotation_offset = Quaternions.from_transforms(
            rot_mat[np.newaxis, :, :]
        )
        ...
        for track in X:
            if self.method == "abdolute_translation_deltas":
                ...
                if self.position_smoothing > 0:
                    x_sm = filters.gaussian_filter1d(
                        x, self.position_smoothing, axis=0, mode="nearest"
                    )
                    z_sm = filters.gaussian_filter1d(
                        z, self.position_smoothing, axis=0, mode="nearest"
                    )
            ...
            # end of abdolute_translation_deltas
                ...
                quats = Quaternions.from_euler(
                    rotations, order=rot_order.lower(), world=False
                )
                    ...
                    np.array(
                        [
                            t3d.euler.quat2euler(
                                q, axes=("s" + rot_order.lower()[::-1])
                            )[::-1]
                            for q in new_rotations
                        ]
                    )
                    * 180.0
                    / np.pi
                ...
                if trajectory_filterwidth > 0:
                    reference = filters.gaussian_filter1d(
                        positions, trajectory_filterwidth, axis=0, mode="nearest"
                    )
    ...
    def inverse_transform(self, X, copy=None, start_pos=None):
        ...
        for track in X:
            ...
            if self.method == "abdolute_translation_deltas":
                ...
                # recz = dz[:-1] + dz[1:]
                if self.position_smoothing > 0:
                    new_df[xpcol] = pd.Series(
                        data=new_df[xpcol] + recx, index=new_df.index
                    )
                    new_df[zpcol] = pd.Series(
                        data=new_df[zpcol] + recz, index=new_df.index
                    )
                else:
            ...
            # end of abdolute_translation_deltas
                ...
                for i in range(dx.shape[0] - 1):
                    # print(dry[i])
                    q_y = Quaternions.from_angle_axis(
                        np.array(dry[i + 1]), np.array([0, 1, 0])
                    )
                ...
                if self.separate_root:
                    ref_rot_order = "ZXY"
                    new_df["reference_Xposition"] = pd.Series(
                        data=rec_xp, index=new_df.index
                    )
                    new_df["reference_Zposition"] = pd.Series(
                        data=rec_zp, index=new_df.index
                    )
                    eulers_ry = (
                        np.array(
                            [
                                t3d.euler.quat2euler(
                                    q, axes=("s" + ref_rot_order.lower()[::-1])
                                )[::-1]
                                for q in rec_ry
                            ]
                        )
                    ...
                    new_df["reference_Yrotation"] = pd.Series(
                        data=eulers_ry[:, ref_rot_order.find("Y")], index=new_df.index
                    )
                ...
                if self.separate_root:
                    ...
                    new_df["reference_Yposition"] = pd.Series(
                        data=rec_yp, index=new_df.index
                    )
                    new_df["reference_Zposition"] = pd.Series(
                        data=rec_zp, index=new_df.index
                    )
...
class RootCentricPositionNormalizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            all_but_root = [
                joint for joint in track.skeleton if track.root_name not in joint
            ]
            # all_but_root = [joint for joint in track.skeleton]
            for joint in all_but_root:
                new_df["%s_Xposition" % joint] = pd.Series(
                    data=track.values["%s_Xposition" % joint] - projected_root_pos[rxp],
                    index=new_df.index,
                )
                new_df["%s_Yposition" % joint] = pd.Series(
                    data=track.values["%s_Yposition" % joint] - projected_root_pos[ryp],
                    index=new_df.index,
                )
                new_df["%s_Zposition" % joint] = pd.Series(
                    data=track.values["%s_Zposition" % joint] - projected_root_pos[rzp],
                    index=new_df.index,
                )
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.const_values_ = {
            c: X[0].values[c].values[0] for c in cols if (stds[c] < self.eps).any()
        }
        return self
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            if self.is_DataFrame:
                ...
                normalized_track.values = (
                    track.values - self.data_mean_
                ) / self.data_std_
            else:
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            if self.is_DataFrame:
                ...
                unnormalized_track.values = (
                    track.values * self.data_std_
                ) + self.data_mean_
            else:
...
class ListMinMaxScaler(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            if self.is_DataFrame:
                normalized_track = track.copy()
                normalized_track.values = (track.values - self.data_min_) / (
                    self.data_max_ - self.data_min_
                )
            else:
                normalized_track = (track - self.data_min_) / (
                    self.data_max_ - self.data_min_
                )
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            if self.is_DataFrame:
                unnormalized_track = track.copy()
                unnormalized_track.values = (
                    track.values * (self.data_max_ - self.data_min_)
                ) + self.data_min_
            else:
                unnormalized_track = (
                    track * (self.data_max_ - self.data_min_)
                ) + self.data_min_
...
class Resampler(BaseEstimator, TransformerMixin):
    ...
    def resample_df(self, df, new_frametime, old_frametime, mode="cubic"):
        ...
        for jj in range(data.shape[1]):
            ...
            f = interpolate.interp1d(
                x, y, bounds_error=False, kind=mode, fill_value="extrapolate"
            )
            data_out[:, jj] = f(xnew)

        time_index = (
            pd.to_timedelta([f for f in range(xnew.shape[0])], unit="s") * new_frametime
        )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # else:
            new_track.values = self.resample_df(
                track.values, self.tgt_frametime, track.framerate, self.method
            )
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            ...
            if self.method == "resample_poly":
                new_track.values = self.resample_poly_df(
                    track.values, self.orig_frametime, track.framerate
                )
            else:
                new_track.values = self.resample_df(
                    track.values, self.orig_frametime, track.framerate, self.method
                )
