...
class BVHParser:
    ...
    def _read_channels(self, bvh, token_index):
        ...
        for i in range(channel_count):
            ...
            token_index = token_index + 1
            if (
                channels[i] == "Xrotation"
                or channels[i] == "Yrotation"
                or channels[i] == "Zrotation"
            ):
                order += channels[i][0]
            else:
                order = ""
    ...
    def _parse_hierarchy(self, bvh):
        ...
        channels, self.current_token, order = self._read_channels(
            bvh, self.current_token
        )
        ...
        while (
            bvh[self.current_token][1] == "JOINT" or bvh[self.current_token][1] == "End"
        ):
        ...
        for i in range(stop):
            ...
            for channel in self._motion_channels:
                channel_values.append(
                    (channel[0], channel[1], float(bvh[self.current_token][1]))
                )
                self.current_token = self.current_token + 1
